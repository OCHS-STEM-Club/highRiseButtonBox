...
//deadband
	if (stick->GetRawAxis(1) < 0.05 and stick->GetRawAxis(1) > -0.05) {
		...
		x = stick->GetRawAxis(1);// drive
	}
	else {
		x = stick->GetRawAxis(1) * 0.85; //DNE 0.85 for less jank
	}

	if (stick->GetRawAxis(2) < 0.05 and stick->GetRawAxis(2) > -0.05) {
		z = 0;
	}
	else if(stick->GetRawButton(4)) { // turn
		z = stick->GetRawAxis(2)*0.75; //DNE 0.75 for less jank
	}
	else{
