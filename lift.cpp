...
//human control
	//if(xbox->GetRawButton(5) and ((limit->Get() and limit2->Get()) or ((!limit->Get() and xb > 0) or (!limit2->Get() and xb < 0)))){
		...
		if (bb->GetRawAxis(23) or bb->GetRawButton(22)) {
			srxLiM->Set(ControlMode::PercentOutput, -0.5);
		}
		if (bb->GetRawButton(25) or bb->GetRawButton(26)) {
			srxLiM->Set(ControlMode::PercentOutput, 0.5);
		}
//	}
	//pid control
	if(bb->GetRawButton(18)) {
		srxLiM->Set(ControlMode::Position, scaleheight); //Scale Height -2 Just Incase
		timer->Reset();
		goback = false;
		liftlocation = 0;
	}
	if (bb->GetRawButton(20)) {
		srxLiM->Set(ControlMode::Position, switchheight); //Switch Height
		timer->Reset();
		goback = false;
		liftlocation = 90;
	}
	if(bb->GetRawButton(8)){
		srxLiM->Set(ControlMode::Position, driveheight); //Just Above Ground
		timer->Reset();
		goback = false;
		liftlocation = 270;
	}
	if (bb->GetRawButton(15)) {
...
//	if(!xbox->GetRawButton(5) && !xbox->GetRawButton(6)){
		if(liftlocation == 180){
			if(bb->GetRawButton(5) || bb->GetRawButton(10) || bb->GetRawButton(3)){
